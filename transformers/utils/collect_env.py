...
def collect_env_info():
    ...
    try:
        ...
        data.append(
            (
                "transformers",
                transformers.__version__ + " @" + os.path.dirname(transformers.__file__),
            )
        )
    except ImportError:
        data.append(("transformers", "failed to import"))

    try:
        import transformers._C as _C
    except ImportError as e:
        data.append(("transformers._C", f"not built correctly: {e}"))
        ...
        if has_cuda:
            data.append(
                ("transformers arch flags", detect_compute_compatibility(CUDA_HOME, _C.__file__))
            )
